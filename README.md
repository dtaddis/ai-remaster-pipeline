...
**Wan 2.1 VACE (14B)** is a separate model family behind the same controls. It uses a Q4_K_M GGUF transformer, the UMT5 text encoder, and the lightx2v step-distill LoRA (about 19 GB, none gated, fetched on first use). Wan is trained on 81-frame clips, so ARP renders each chunk as a sequence of 81-frame VACE windows: every window opens with finished frames that VACE keeps and continues from. When the previous-chunk start guide is on, a chunk likewise opens with the previous chunk's overlap frames rather than a single guide frame. Chunk length and the chunk plan therefore work as they do for LTX. Windows never span a shot change: ARP finds cuts with the Shot Detection settings and starts a fresh window at each one, because VACE would otherwise carry the previous shot's surroundings into the new shot. The masks, trim/extend geometry, custom mask, all-black-regions mode, seed guides, and Recomposition are shared with the LTX path. Guide frames fill the generated region of their frame, and their strength sets how firmly VACE keeps them. Wan's prompt should describe the scene; at its CFG-free setting the negative prompt has no effect.
