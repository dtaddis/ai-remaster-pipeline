...
**Wan 2.1 VACE (14B)** is a separate model family behind the same controls. It uses a Q4_K_M GGUF transformer, the UMT5 text encoder, and the lightx2v step-distill LoRA (about 19 GB, none gated, fetched on first use). A 20-second chunk is far too large for one Wan pass, so ARP renders each chunk as a sequence of VACE windows, and every window opens with finished frames that VACE keeps and continues from. **Wan window length** sets how many frames each window sees. The default, 161 frames, gives about 6.7 seconds of look-ahead at 24 fps and kept invented surroundings steadier than 81-frame windows in side-by-side tests, for about 1.6 times the render time. It fits in 24 GB at 720p; larger canvases automatically use shorter windows to stay within VRAM. When the previous-chunk start guide is on, a chunk likewise opens with the previous chunk's overlap frames rather than a single guide frame. Chunk length and the chunk plan therefore work as they do for LTX. Windows never span a shot change: ARP finds cuts with the Shot Detection settings and starts a fresh window at each one, because VACE would otherwise carry the previous shot's surroundings into the new shot. The masks, trim/extend geometry, custom mask, all-black-regions mode, seed guides, and Recomposition are shared with the LTX path. Guide frames fill the generated region of their frame, and their strength sets how firmly VACE keeps them. Wan's prompt should describe the scene (ARP drops LTX's `outpaint` trigger word, which Wan reads as a request to paint); at its CFG-free setting the negative prompt has no effect.
