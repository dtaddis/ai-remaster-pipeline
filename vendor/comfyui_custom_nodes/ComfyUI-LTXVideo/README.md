...
> **ARP bundled patch:** `guide_attention_patch.py` partitions LTX guide queries into generated, full-strength guide, and soft-strength guide groups. Full-strength video-guide rows use ordinary unmasked attention, so only genuinely attenuated still-guide rows allocate the large per-row mask. It also evaluates the token-independent LTX feed-forward layers in 4,096-token slices, avoiding a multi-gigabyte four-times-width GELU tensor on long guided videos. Both changes preserve the model's conditioning semantics; `ARP_LTX_FF_CHUNK_TOKENS` can override the feed-forward slice size.
