...
class PipelineApp:
    ...
    def load_project(self) -> dict[str, str]:
        ...
        with self.source_analysis_lock:
            self.source_tone_default_keys.clear()
        self.restore_image_sequence_source()
        self.adopt_saved_project_artifacts()
        self.hydrate_stage_inputs("")
        self.save()
        self.log.append(f"Loaded ARP project: {path}")
        return {"path": str(path)}

    def adopt_saved_project_artifacts(self) -> None:
        """Carry a project's saved work over to the names this build derives.

        Artifact names hash their identity, so any identity change (a new geometry version, a
        renamed section clip, a new container) renames everything downstream. A project still
        records the paths it used, so a saved file whose current-name counterpart is missing is
        adopted under the new name instead of the work silently disappearing. Must run before
        hydrate_stage_inputs, which overwrites these saved paths with derived ones.
        """
        ...
        outpaint_values = settings.get("outpaint", {})
        saved_outpainted = settings.get("recomp", {}).get("outpainted_video", "") or settings.get("shots", {}).get("outpainted_video", "")
        saved_shots = next((settings.get(key, {}).get("manifest", "") for key in ("references", "recomp", "colour") if settings.get(key, {}).get("manifest", "")), "")
        ...
        chunk_pair = pairs[0]
        for saved, current in pairs:
            # A default chunk plan generated by an earlier open must not hide the saved one.
            replace_existing = (
                (saved, current) == chunk_pair
                and bool(saved) and resolve(saved).is_file() and resolve(current).is_file()
                and chunk_manifest_has_user_edits(resolve(saved))
                and not chunk_manifest_has_user_edits(resolve(current))
            )
            try:
                adopted = adopt_saved_artifact(saved, current, replace_existing)
            except (OSError, subprocess.CalledProcessError) as exc:
                self.log.append(f"Could not carry {saved} over to {current}: {exc}")
                continue
            if not adopted:
                continue
            self.log.append(f"Carried older project file over to the current name: {saved} -> {adopted}")
            if adopted == current_shots and resolve(current_outpainted).exists():
...
def chunk_manifest_has_user_edits(path: Path) -> bool:
    """True when any chunk row carries something the user chose rather than a generated default."""
    for row in read_outpaint_chunk_rows(path).values():
        if any(str(row.get(key) or "").strip() for key in (
            "custom_seconds", "prompt_suffix", "negative_suffix",
            "guide_image", "guide_end_image", "guide_frames", "anchor_image",
        )):
            return True
        if str(row.get("offset_mode") or "inherit") != "inherit" or str(row.get("auto_start_guide") or "true").lower() == "false":
            return True
    return False


def adopt_saved_artifact(saved_text: str, current_text: str, replace_existing: bool = False) -> str:
    """Copy a file saved under an older name to its current name; return that name if adopted.

    An existing current-name file wins unless replace_existing is set, in which case it is kept
    beside the adopted file as .bak. A container change (older .mp4 renders, current .mkv
    names) is a lossless stream-copy remux, so the file's contents match its extension.
    """
    if not saved_text or not current_text:
        return ""
    saved, current = resolve(saved_text), resolve(current_text)
    if saved == current or not saved.is_file() or (current.exists() and not replace_existing):
        return ""
    current.parent.mkdir(parents=True, exist_ok=True)
    partial = current.with_name(f"{current.stem}.partial{current.suffix}")
    if saved.suffix.lower() == current.suffix.lower():
        ...
        if not ffmpeg:
            return ""
        subprocess.run(
            [ffmpeg, "-y", "-v", "error", "-i", str(saved), "-map", "0:v", "-map", "0:a?", "-c", "copy", str(partial)],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
        )
    if current.exists():
        current.replace(current.with_name(current.name + ".bak"))
    partial.replace(current)
    return rel(current)
